...
class TaoEval:
    def __init__(self,
        ...
        if not logger:
            ...
            self.logger = logger
        self.use_gt_ignore = use_gt_ignore
    ...
    def _prepare(self):
        ...
        # set ignore flag
        gts = self.tao_gt.group_ann_tracks(gt_anns, use_gt_ignore=self.use_gt_ignore)
        dts = self.tao_dt.group_ann_tracks(dt_anns, use_gt_ignore=False)
        for gt in gts:
            if "ignore" not in gt:
                gt["ignore"] = 0
