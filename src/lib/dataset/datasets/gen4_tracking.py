...
class Gen4Tracking(VideoDataset):
  num_categories = 7
  dataset_folder = 'gen4'
  default_resolution = [384, 640]
  class_name = ['Pedestrian', 'Two_Wheeler', 'Car', 'Truck', 'Bus', 'Traffic_Sign', 'Traffic_Light']
  ...
  def __init__(self, opt, split, rank=None):
    ...
    ann_path = os.path.join(
      data_dir, opt.annotations_dir, 'tracking_{}.json'.format(
        ann_file_))
    self.images = None
    super(Gen4Tracking, self).__init__(opt, split, ann_path, img_dir)

    self.box_size_thresh = [100] * 7
